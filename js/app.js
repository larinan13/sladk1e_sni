...
function isValidEmail(email) {
    // Формат: локальная_часть@домен.домен_верхнего_уровня
    const re = /^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    position: 'auto left'       // ← позиционирование
        ...
        document.getElementById('bookingForm').addEventListener('submit', function(e) {
            ...
            // Проверка email — усиленная
            if (data.email && !isValidEmail(data.email.trim())) {
                errors.push('Некорректный формат email. Пример: ivan@mail.ru');
            }
            
            // Проверка телефона — должен быть полностью заполнен
            if (data.phone && data.phone.replace(/\D/g, '').length !== 11) {
                errors.push('Телефон должен быть введён полностью в формате +7(999)999-99-99');
            }
